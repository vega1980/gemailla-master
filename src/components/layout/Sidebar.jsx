...
export default function Sidebar({ collapsed, setCollapsed }) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-40 flex h-full flex-col transition-all duration-300 ${collapsed ? 'w-16' : 'w-72'}`}
      style={{
        background: 'linear-gradient(180deg, rgba(1,12,14,0.995) 0%, rgba(2,24,26,0.99) 56%, rgba(1,10,12,0.995) 100%)',
        borderRight: '1px solid rgba(216,166,62,0.68)',
        boxShadow: '16px 0 48px rgba(0,0,0,0.44), 1px 0 24px rgba(26,213,211,0.12)',
      }}
    >
      <div
        className="flex items-center justify-center border-b px-4 py-5"
        style={{ borderColor: 'rgba(214,163,57,0.34)' }}
      >
        {!collapsed ? (
          <div className="flex flex-col items-center gap-2 text-center">
            <img
              ...
              style={{ filter: 'drop-shadow(0 0 16px rgba(222,170,62,0.38))' }}
            />
            <span className="gold-title font-display text-xl font-bold tracking-[0.12em]">GEMAILLA IA</span>
            <p className="max-w-[15rem] text-[0.62rem] uppercase leading-4 tracking-[0.16em]" style={{ color: 'rgba(239,205,126,0.82)' }}>
      ...
      {!collapsed && activeCompany && (
        <div className="border-b px-3 py-3" style={{ borderColor: 'rgba(214,163,57,0.22)' }}>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                style={{
                  background: 'linear-gradient(135deg, rgba(14,131,132,0.2), rgba(211,157,48,0.09))',
                  border: '1px solid rgba(43,220,218,0.3)',
                  boxShadow: 'inset 0 0 18px rgba(43,220,218,0.04)',
                }}
              >
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all duration-200 ${collapsed ? 'justify-center' : ''}`}
              style={isActive ? {
                background: 'linear-gradient(90deg, rgba(20,213,211,0.28), rgba(20,213,211,0.08))',
                border: '1px solid rgba(43,225,223,0.62)',
                color: '#effffc',
                boxShadow: '0 0 20px rgba(43,225,223,0.18), inset 3px 0 0 #28e0de',
              } : {
                color: 'rgba(221,232,226,0.76)',
            ...
            >
              <item.icon
                ...
                style={{
                  color: isActive ? '#69fbf6' : '#2bd4d2',
                  filter: isActive ? 'drop-shadow(0 0 8px rgba(55,235,232,0.68))' : 'none',
                }}
              />
              {!collapsed && <span className="whitespace-nowrap text-[0.82rem] font-semibold tracking-wide">{item.label}</span>}
            </Link>
          );
        })}
      </nav>
      ...
      <button
        ...
        style={{ background: '#061719', border: '1px solid rgba(225,174,66,0.75)', color: '#e1b74d' }}
        aria-label={collapsed ? 'Expandir menú' : 'Contraer menú'}
      >
        {collapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronLeft className="h-3 w-3" />}
      </button>

      <div className="border-t p-3" style={{ borderColor: 'rgba(214,163,57,0.26)' }}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
          {!collapsed && (
            <div>
              <p className="text-xs font-semibold" style={{ color: '#e9d49f' }}>Sesión activa</p>
              <p className="text-[0.65rem]" style={{ color: 'rgba(57,214,211,0.76)' }}>Entorno empresarial</p>
            </div>
          )}
          <button
            ...
            style={{ color: '#d9a743', border: '1px solid rgba(217,167,67,0.3)' }}
            title="Cerrar sesión"
          >
