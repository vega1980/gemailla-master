...
test('aiHandler usa exclusivamente vertex-gemini y no consulta OPENAI_API_KEY', async () => {
...
test('aiHandler normaliza integration=openai a gemailla-ai', async () => {
  const store = seedStore();
  const { aiHandler } = loadAiHandler({
    store,
    config: createConfig(),
...
test('aiHandler falla de forma controlada para proveedor desconocido y no hace fallback', async () => {
  ...
  const { aiHandler } = loadAiHandler({
    store,
    config: createConfig({ provider: 'anthropic', model: 'claude-x' }),
    geminiImpl: async () => {
      geminiCalls += 1;
      return { outputText: 'x', provider: 'vertex-gemini', model: 'gemini-3.6-flash', usage: {}, usageAvailable: false };
    },
  });
...
test('aiHandler autentica y autoriza antes de Vertex, conserva aislamiento multiempresa y rechazo de documentos cruzados', async () => {
  let providerCalls = 0;
...
test('aiHandler procesa JSON estructurado en aiHandler y rechaza JSON inválido o texto adicional', async () => {
...
test('aiHandler conserva la reserva cuando falta usageMetadata de vertex-gemini y no inventa costos', async () => {
  const store = seedStore();
  const { aiHandler } = loadAiHandler({
    store,
    config: createConfig(),
...
test('aiHandler falla antes de llamar a Vertex cuando falta pricing aprobado para el modelo', async () => {
  const store = seedStore();
  let geminiCalls = 0;
  ...
  const { aiHandler } = loadAiHandler({
    ...
    config: createConfig({
      providers: {
        ...
        'vertex-gemini': {
          ...baseConfig.providers['vertex-gemini'],
          pricing: {},
        },
      },
    }),
  ...
  assert.match(res.payload.error, /Falta configuracion aprobada de precios para vertex-gemini\/gemini-3.6-flash/);
  assert.equal(geminiCalls, 0);
  assert.equal(collectDocs(store, 'aiUsage').length, 0);
});

test('aiHandler falla antes de llamar a Vertex cuando falta configuración obligatoria del modelo', async () => {
  let geminiCalls = 0;
  const baseConfig = createConfig();
  const { aiHandler } = loadAiHandler({
    config: createConfig({
  ...
  assert.equal(res.statusCode, 503);
  assert.match(res.payload.error, /modelo exacto aprobado/);
  assert.equal(geminiCalls, 0);
});

test('aiHandler usa el modelo válido del proveedor cuando runtimeConfig.model está vacío', async () => {
  let geminiCalls = 0;
  const { aiHandler } = loadAiHandler({
