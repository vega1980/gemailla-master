...
export async function clearFirestoreEmulator({ projectId = getE2EProjectId() } = {}) {
  const firestoreBaseUrl = normalizeEmulatorHost(
...
export async function clearAuthEmulator({ projectId = getE2EProjectId() } = {}) {
  const authBaseUrl = normalizeEmulatorHost(
...
export async function clearFirebaseEmulators(options) {
  await Promise.all([
    clearFirestoreEmulator(options),
    clearAuthEmulator(options),
  ]);
}
